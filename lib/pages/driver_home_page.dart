...
class _DriverHomeScreenState extends State<DriverHomeScreen> {
  ...
  Future<void> _onRideRequestSelected(RideRequest request) async {
    try {
      ...
      LatLng driverCoords = LatLng(driverPosition.latitude, driverPosition.longitude);

      // Get the route from driver to pickup
      PolylineResult result = await _polylinePoints.getRouteBetweenCoordinates(
        _googleMapsApiKey,
        PointLatLng(driverCoords.latitude, driverCoords.longitude),
        PointLatLng(request.pickupCoords.latitude, request.pickupCoords.longitude),
        travelMode: TravelMode.driving,
      );

      if (result.points.isEmpty) {
        throw Exception(result.errorMessage ?? "Could not calculate route");
      }
      ...
      if(mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Error plotting route: $e")));
      }
    }
  }
  ...
  Future<void> _animateCameraToFitRoute(LatLng point1, LatLng point2) async {
    ...
    LatLngBounds bounds = LatLngBounds(
      southwest: LatLng(min(point1.latitude, point2.latitude), min(point1.longitude, point2.longitude)),
      northeast: LatLng(max(point1.latitude, point2.latitude), max(point1.longitude, point2.longitude)),
    );
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            leading: const Icon(Icons.person_pin_circle, color: Color(0xFFFFD700), size: 32),
                            title: Text(
